...
import { WarningIcon } from "../components/mdx/icons/WarningIcon";
import { WrenchIcon } from "../components/mdx/icons/WrenchIcon";
import { LockUnlockedIcon } from "../components/mdx/icons/LockUnlockedIcon";
import { TopicsIcon } from "../components/mdx/icons/TopicsIcon";
import { KeyIcon } from "../components/mdx/icons/KeyIcon";
...
| Value      | Name          | Detection keys in window global                        |
| ---------- | ------------- | ------------------------------------------------------ |
...
| Email ^3^               | `<USERNAME@DOMAIN>`            | `<nelly@discordapp.com>`        |
...
^3^ Can be optionally prefixed with `mailto:`, following the same format as the [`mailto:`](https://en.wikipedia.org/wiki/Mailto) URI scheme. Does not support multiple comma-separated addresses. Supports headers (e.g. `<nelly@discord.com?subject=Title&body=Message%20Content>`).
...
| Application Directory Collection | /application-directory/collection-items/[\{application.id\}](/resources/application#application-object)/[\{image_hash\}](/resources/application-directory#application-directory-collection-item-structure).png                                     | PNG, WEBP                 |
...
| Store Asset                      | /app-assets/[\{application_id\}](/resources/application#application-object)/store/[\{asset_id\}](/resources/store#store-asset-object).png                                                                                                          | PNG, JPEG, WebP           |
...
Attachments uploaded to Discord's CDN (like user-uploaded images) have signed URLs with a preset expiry time. This includes ephemeral attachments. Discord automatically refreshes attachment CDN URLs that appear within the client,
so when you receive a payload with a signed URL (like when you [fetch a message](/resources/message#get-messages)), it will be valid. Note that this does not apply to URLs present in user-generated content, such as message content or embed descriptions.
...
To upload a set of files larger than this, you must utilize [Google Cloud uploads](#uploading-to-google-cloud).
...
This allows you to upload up a file up to **500 MiB** directly to Google Cloud. Note that [file size limits](#uploading-files) will still apply when sending the attachment in a message.
...
In API v8 and above, most API resources have switched away from this format, instead opting for `<string>` and `<string>_localizations` fields.
...
###### MFA Required<KeyIcon className="ml-2 inline-block h-4 w-4 align-text-bottom" />
...
###### Audit Log Reason<TopicsIcon className="ml-2 inline-block h-4 w-4 align-text-bottom" />
...
###### Unauthenticated Request<LockUnlockedIcon className="ml-2 inline-block h-4 w-4 align-text-bottom" />

These requests do not require the `Authorization` header to be present.

###### OAuth2 Request<WrenchIcon className="ml-2 inline-block h-4 w-4 align-text-bottom" />
...
###### Deprecated Endpoint<WarningIcon className="ml-2 inline-block h-4 w-4 align-text-bottom" />
